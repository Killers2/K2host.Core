...
'| Use: General (Written by Peter O. in 2013.)          |
' \====================================================/
*/
...
namespace K2host.Core.Classes
{
    ...
    public sealed class OQueryStringHelper
    {

        private OQueryStringHelper() 
        { 
        ...
        private static string[] SplitAt(string s, string delimiter)
        {
            ...
            if (delimiter == null || delimiter.Length == 0)
                throw new ArgumentException("There was no delimiter");
        ...
        private static string PercentDecodeUTF8(string str)
        {
            ...
            for (int i = 0; i < len; ++i)
            {
                ...
                if (c == '%')
                    ...
                    return null; // Non-ASCII characters not allowed
            }
        ...
        private static void ConvertLists(IList<Object> dict)
        {
            ...
            {
                IDictionary<string, Object> value = (dict[i] is IDictionary<string, Object> dictionary) ? dictionary : null;

                // A list contains only indexes 0, 1, 2, and so on, with no gaps.
                ...
                if (IsList(value))
                {
                    ...
                    ConvertLists(value); // Convert the list's descendents if they are lists
        ...
        private static void ConvertLists(IDictionary<string, Object> dict)
        {
            foreach (var key in new List<string>(dict.Keys))
            {
                ...
                IDictionary<string, Object> value = (dict[key] is IDictionary<string, Object> dictionary) ? dictionary : null;
                ...
                else if (value != null)
                {
                    try
                    {
                        ConvertLists(value); // Convert the dictionary's descendents if they are lists
                    }
                    catch (Exception)
                    {
                        throw;
        ...
        public static IDictionary<string, Object> QueryStringToDict(string query, string delimiter)
        {
            ...
            foreach (var keyvalue in ParseQueryString(query, delimiter))
            {
                ...
                {
                    if (!leaf.ContainsKey(path[i]))  // node doesn't exist so add it
                    {
                    ...
                    {
                        IDictionary<string, Object> o = (leaf[path[i]] is IDictionary<string, Object> dictionary) ? dictionary : null;
                        if (o != null)
                        {
                            leaf = o;
                        }
            ...
            // Convert array-like dictionaries to ILists can cause problems with dupe items in other lists
            ConvertLists(root);
